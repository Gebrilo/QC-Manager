...
def test_task_creation_and_listing():
    ...
    try:
        ...
        if projects and len(projects) > 0:
            ...
            create_proj_resp = requests.post(f"{BASE_URL}/api/projects", json=project_payload, headers=HEADERS, timeout=TIMEOUT)
        ...
        resources_resp = requests.get(f"{BASE_URL}/api/resources", headers=HEADERS, timeout=TIMEOUT)
        ...
        list_tasks_resp = requests.get(f"{BASE_URL}/api/tasks", headers=HEADERS, timeout=TIMEOUT)
