...
import { fetchApi, taskTestCaseLinksApi, userStoriesApi, type UserStory } from '@/lib/api';
...
export default function TaskDetailPage() {
    ...
    const [parentStory, setParentStory] = useState<UserStory | null>(null);
    ...
    useEffect(() => {
        if (!task?.parent_user_story_id) {
            setParentStory(null);
            return;
        }

        let mounted = true;
        userStoriesApi.get(task.parent_user_story_id)
            .then(story => {
                if (mounted) setParentStory(story);
            })
            .catch(() => {
                if (mounted) setParentStory(null);
            });
    ...
    const isAiIntakeTask = task.generated_by_ai || task.source === 'ai_intake';
                    ...
                    {isAiIntakeTask && (
                        <QCCard>
                            ...
                            <div className="space-y-0">
                                <DetailRow label="Generated by AI" value="Yes" />
                                <DetailRow label="Channel" value={task.source || 'ai_intake'} />
                                <DetailRow label="Source Skill" value={parentStory?.ai_skill_name || 'Unknown'} />
                                <DetailRow label="Source Agent" value={parentStory?.ai_source_agent || 'Unknown'} />
                                <DetailRow
                                    label="User Story"
                                    value={parentStory?.title || task.parent_user_story_id || 'Unknown'}
                                />
                            </div>
                            {task.parent_user_story_id && (
                                <Link
                                    href={artifactPath('user_story', parentStory || { id: task.parent_user_story_id })}
                                    className="mt-3 inline-flex text-xs font-medium text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300"
                                >
