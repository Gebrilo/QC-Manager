...
describe('Access engine — expanded catalog (issue #80)', () => {
    ...
    test('decorative and redundant _own keys are pruned from the catalog', () => {
        const pruned = [
            ...
            'qc.bugs.edit_own', 'qc.bugs.delete_own',
    ...
    test('pm can create and edit bugs and user stories', () => {
    ...
    test('contributor remains preparation-only and has only own bug visibility in quality work', () => {
        ...
        expect(BUILT_IN_ROLE_PERMISSION_DEFAULTS.contributor).toContain(PERMISSIONS.BUGS_VIEW_OWN);
    });

    test('viewer can view bugs and test cases', () => {
        expect(canUserPerform({ role: 'viewer' }, PERMISSIONS.BUGS_VIEW)).toBe(true);
        expect(canUserPerform({ role: 'viewer' }, PERMISSIONS.TESTCASES_VIEW)).toBe(true);
    });

    test('team_manager can delete own and team test artifacts', () => {
