...
describe('ADR 0009 reporting views', () => {
    test('final project/resource/dashboard views aggregate task effort from the assignment junction (#200)', () => {
        const block = sourceAfter('DROP VIEW IF EXISTS v_projects_with_metrics CASCADE');
    ...
    test('v_tasks_with_metrics derives slot columns from LATERAL joins on the junction (#200)', () => {
        ...
        expect(block).toMatch(/LATERAL \([\s\S]*assignment_type = 'PRIMARY'[\s\S]*\) pri/);
        expect(block).toMatch(/LATERAL \([\s\S]*assignment_type = 'SECONDARY'[\s\S]*\) sec/);
        expect(block).toMatch(/COALESCE\(pri\.estimate_hrs, 0\) AS r1_estimate_hrs/);
        expect(block).toMatch(/COALESCE\(sec\.estimate_hrs, 0\) AS r2_estimate_hrs/);
        ...
        expect(block).not.toMatch(/t\.r1_estimate_hrs|t\.r2_estimate_hrs/);
    });
});
