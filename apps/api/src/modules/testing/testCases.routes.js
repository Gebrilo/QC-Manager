...
const { emitToTuleap: emitTestCase } = require('../integration/services/emitters/test_case');
...
router.get('/', requireAuth, requirePermission('qc.testcases.view'), async (req, res, next) => {
    try {
        ...
        const projectIds = parseCsvParam(req.query.project_ids || project_id);
        const statuses = parseCsvParam(req.query.statuses || status);
        const priorities = parseCsvParam(req.query.priorities || priority);
        ...
        if (projectIds.length > 0) { query += ` AND project_id = ANY($${pn++}::uuid[])`; params.push(projectIds); }
        if (statuses.length > 0) { query += ` AND status = ANY($${pn++}::text[])`; params.push(statuses); }
        if (priorities.length > 0) { query += ` AND priority = ANY($${pn++}::text[])`; params.push(priorities); }
        ...
        if (source === 'local') { query += ` AND tuleap_artifact_id IS NULL`; }
        if (source === 'tuleap') { query += ` AND tuleap_artifact_id IS NOT NULL`; }

        if (req.query.created_from) { query += ` AND created_at >= $${pn++}::date`; params.push(req.query.created_from); }
        if (req.query.created_to) { query += ` AND created_at < ($${pn++}::date + INTERVAL '1 day')`; params.push(req.query.created_to); }
        if (req.query.updated_from) { query += ` AND updated_at >= $${pn++}::date`; params.push(req.query.updated_from); }
        if (req.query.updated_to) { query += ` AND updated_at < ($${pn++}::date + INTERVAL '1 day')`; params.push(req.query.updated_to); }
        ...
        if (relatedId) {
            if (relatedType === 'task') {
                query += ` AND EXISTS (
                    ...
                    WHERE ttc.test_case_id = v_test_case_summary.id AND ttc.task_id = $${pn++}::uuid
                )`;
                ...
                query += ` AND EXISTS (
                    ...
                    WHERE ttc.test_case_id = v_test_case_summary.id AND t.parent_user_story_id = $${pn++}::uuid
                )`;
                params.push(relatedId);
                ...
                query += ` AND (
                    EXISTS (
                        ...
                        WHERE tc.id = v_test_case_summary.id
                          AND tc.linked_bug_ids @> ARRAY[$${pn}::uuid]
                    )
                    OR EXISTS (
                        SELECT 1 FROM bug_test_cases btc
                        JOIN bugs b ON b.id = btc.bug_id AND b.deleted_at IS NULL
                        WHERE btc.test_case_id = v_test_case_summary.id
                    )
                )`;
                params.push(relatedId);
                pn++;
            } else if (relatedType === 'suite') {
                query += ` AND EXISTS (
                    SELECT 1 FROM test_suite_cases tsc
                    JOIN test_suites ts ON ts.id = tsc.suite_id
                    WHERE tsc.test_case_id = v_test_case_summary.id
