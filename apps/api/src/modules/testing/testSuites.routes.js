...
async function validateSuiteTestCases(client, qcProjectId, testCaseIds) {
    ...
    const crossProjectIds = casesResult.rows
        .filter(row => row.project_id !== qcProjectId)
        .map(row => row.id);
...
router.get('/', requireAuth, requirePermission('qc.testsuites.view'), async (req, res, next) => {
    try {
        ...
        const suiteTypes = parseCsvParam(req.query.suite_types || req.query.suite_type);
        const readinessScopes = parseCsvParam(req.query.readiness_scopes || req.query.readiness_scope);
        ...
        if (projectIds.length > 0) { whereClauses.push(`ts.project_id = ANY($${pn++}::uuid[])`); params.push(projectIds); }
        if (statuses.length > 0) { whereClauses.push(`ts.status = ANY($${pn++}::text[])`); params.push(statuses); }
        if (suiteTypes.length > 0) { whereClauses.push(`ts.suite_type = ANY($${pn++}::text[])`); params.push(suiteTypes); }
        if (readinessScopes.length > 0) { whereClauses.push(`ts.readiness_scope = ANY($${pn++}::text[])`); params.push(readinessScopes); }
        if (req.query.created_from) { whereClauses.push(`ts.created_at >= $${pn++}::date`); params.push(req.query.created_from); }
        if (req.query.created_to) { whereClauses.push(`ts.created_at < ($${pn++}::date + INTERVAL '1 day')`); params.push(req.query.created_to); }
        if (req.query.updated_from) { whereClauses.push(`ts.updated_at >= $${pn++}::date`); params.push(req.query.updated_from); }
        if (req.query.updated_to) { whereClauses.push(`ts.updated_at < ($${pn++}::date + INTERVAL '1 day')`); params.push(req.query.updated_to); }
        if (relatedId && relatedType === 'test_case') {
            whereClauses.push(`EXISTS (
                SELECT 1 FROM test_suite_cases rel
                WHERE rel.suite_id = ts.id
                  AND rel.snapshot_id IS NULL
                  AND rel.test_case_id = $${pn++}::uuid
            )`);
