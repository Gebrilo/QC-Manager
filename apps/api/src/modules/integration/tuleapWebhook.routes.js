...
async function provisionTuleapProject({ tuleap_project_id, tuleap_project_name, tuleap_short_name, tuleap_tracker_id, tracker_type = 'task' }) {
    ...
    if (tuleap_tracker_id) {
        const existingConfig = await pool.query(
            'SELECT * FROM tracker_config WHERE tuleap_tracker_id = $1 AND tuleap_project_id = $2 AND is_active = true LIMIT 1',
        ...
        if (existingConfig.rows.length > 0) {
            ...
            const configRes = await pool.query(
                `INSERT INTO tracker_config (tuleap_tracker_id, tuleap_project_id, qc_project_id, tracker_type, is_active)
                 VALUES ($1, $2, $3, $4, true)
                 ON CONFLICT DO NOTHING
                 RETURNING *`,
    ...
    try {
        const result = await pool.query(`
            ...
            WHERE wl.tuleap_tracker_id IS NOT NULL
              AND NOT EXISTS (
                SELECT 1 FROM tracker_config c
...
tracker_type,
        ...
        const result = await pool.query(`
            INSERT INTO tracker_config (
                ...
                qc_project_id, artifact_fields, status_value_map, is_active
...
router.put('/config/:id', async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `UPDATE tracker_config SET ${updates.join(', ')} WHERE id = $${paramIdx} RETURNING *`,
            values
        );
...
router.delete('/config/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query(
            'UPDATE tracker_config SET is_active = false, updated_at = CURRENT_TIMESTAMP WHERE id = $1 RETURNING *',
            [id]
        );
        if (result.rows.length === 0) {
            return res.status(404).json({ success: false, error: 'Config not found' });
...
router.post('/unified', async (req, res) => {
    try {
        ...
        let configRes = await pool.query(
            'SELECT * FROM tracker_config WHERE tuleap_tracker_id = $1 AND is_active = true LIMIT 1',
            [tracker_id]
        );
