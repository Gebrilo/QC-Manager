...
const PENDING_LINK_JOIN_TABLES = {
  bugs: { test_case: 'bug_test_cases' },
};
...
async function drainPending({ qcProjectId, justPersistedQcId, justPersistedQcType, justPersistedTuleapId, query }) {
  ...
  for (const table of PENDING_TABLES) {
    ...
    for (const row of selectResult.rows) {
      ...
      const joinTable = (PENDING_LINK_JOIN_TABLES[table] || {})[justPersistedQcType];
      if (joinTable) {
        await query(
          `INSERT INTO ${joinTable} (bug_id, test_case_id, source, relationship_type)
           VALUES ($1, $2, 'tuleap', 'reveals')
           ON CONFLICT (bug_id, test_case_id) DO NOTHING`,
          [row.id, justPersistedQcId]
        );
      }
