...
async function resolveConfig(artifactType, qcProjectId) {
  if (!qcProjectId) return null;
  const dbType = artifactType === 'user-story' ? 'user_story' : artifactType;
  const result = await db.pool.query(
    'SELECT * FROM tracker_config WHERE qc_project_id = $1 AND tracker_type = $2 AND is_active = true LIMIT 1',
    [qcProjectId, dbType]
  );
  return result.rows[0] || null;
}

async function resolveTrackerId(artifactType, qcProjectId) {
  const config = await resolveConfig(artifactType, qcProjectId);
  ...
  if (req.body && req.body.artifact_type) {
    ...
    if (['bug', 'task', 'user_story'].includes(unified.artifact_type)) {
      ...
      const configResult = await db.pool.query(
        `SELECT * FROM tracker_config WHERE qc_project_id = $1 AND tracker_type = $2 AND is_active = true`,
...
async function handleTaskCreate(req, res) {
  ...
  const configResult = await db.pool.query(
    `SELECT * FROM tracker_config WHERE qc_project_id = $1 AND tracker_type = 'task' AND is_active = true`,
    [pid]
  );
  const config = configResult.rows[0];
  if (!config) {
...
async function handleBugCreate(req, res) {
  ...
  const pid = project_id || req.body.project_id;
  const configResult = await db.pool.query(
    `SELECT * FROM tracker_config WHERE qc_project_id = $1 AND tracker_type = 'bug' AND is_active = true`,
    [pid]
  );
  const config = configResult.rows[0];
  if (!config) {
...
async function handleUserStoryCreate(req, res) {
  ...
  const configResult = await db.pool.query(
    `SELECT * FROM tracker_config WHERE qc_project_id = $1 AND tracker_type = 'user_story' AND is_active = true`,
    [pid]
  );
  const config = configResult.rows[0];
  if (!config) {
...
async function handleTestCaseCreate(req, res) {
  ...
  const configResult = await db.pool.query(
    `SELECT * FROM tracker_config WHERE qc_project_id = $1 AND tracker_type = 'test_case' AND is_active = true`,
    [pid]
  );
  const config = configResult.rows[0];
  if (!config) {
...
router.post('/:type', requireAuth, async (req, res) => {
  ...
  const qcProjectId = req.body.project_id;
