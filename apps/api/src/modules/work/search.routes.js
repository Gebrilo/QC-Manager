...
function buildSearchQuery({ allowedTypes, includeArchived, qcProjectId }) {
    const deletedFilter = includeArchived ? '1=1' : 'deleted_at IS NULL';
    const projectFilter = qcProjectId ? 'AND project_id = $3' : '';
    const fragments = [];
...
router.get('/', requireAuth, async (req, res, next) => {
    try {
        ...
        const qcProjectId = req.query.project_id ? String(req.query.project_id) : null;
        ...
        if (qcProjectId) params.push(qcProjectId);

        const result = await pool.query(
            buildSearchQuery({ allowedTypes, includeArchived, qcProjectId }),
            params
        );
