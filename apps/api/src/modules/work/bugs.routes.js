...
const { emitToTuleap: emitBug } = require('../integration/services/emitters/bug');
...
router.get('/', requireAuth, requirePermission('qc.bugs.view'), async (req, res) => {
    try {
        const { limit = 50, offset = 0, sort = 'created_at:desc' } = req.query;
        const projectIds = parseCsvParam(req.query.project_ids || req.query.project_id).filter(validUUID);
        const statuses = parseCsvParam(req.query.statuses || req.query.status);
        const severities = parseCsvParam(req.query.severities || req.query.severity);
        const assignees = parseCsvParam(req.query.assignee_ids || req.query.assignee || req.query.assigned_to);
        const source = ['local', 'tuleap'].includes(req.query.source) ? req.query.source : null;
        const search = String(req.query.q || req.query.search || '').trim();
        const relatedType = req.query.related_type || req.query.related_artifact_type;
        const relatedId = validUUID(req.query.related_id || req.query.related_artifact_id);
        ...
        if (projectIds.length > 0) {
            addWhere(`b.project_id = ANY($${paramIndex}::uuid[])`, projectIds);
        }

        if (statuses.length > 0) {
            addWhere(`b.status = ANY($${paramIndex}::text[])`, statuses);
        }

        if (severities.length > 0) {
            addWhere(`b.severity = ANY($${paramIndex}::text[])`, severities);
        }

        if (assignees.length > 0) {
            addWhere(`b.assigned_to = ANY($${paramIndex}::text[])`, assignees);
        }

        if (source === 'local') {
            addWhere('b.tuleap_artifact_id IS NULL');
        } else if (source === 'tuleap') {
            addWhere('b.tuleap_artifact_id IS NOT NULL');
        }

        if (req.query.created_from) {
            addWhere(`b.created_at >= $${paramIndex}::date`, req.query.created_from);
        }
        if (req.query.created_to) {
            addWhere(`b.created_at < ($${paramIndex}::date + INTERVAL '1 day')`, req.query.created_to);
        }
        if (req.query.updated_from) {
            addWhere(`b.updated_at >= $${paramIndex}::date`, req.query.updated_from);
        }
        if (req.query.updated_to) {
            addWhere(`b.updated_at < ($${paramIndex}::date + INTERVAL '1 day')`, req.query.updated_to);
        }

        if (search) {
            addWhere(`(
                b.title ILIKE $${paramIndex}
                OR b.bug_id ILIKE $${paramIndex}
                OR b.description ILIKE $${paramIndex}
                OR b.component ILIKE $${paramIndex}
                OR b.assigned_to ILIKE $${paramIndex}
                OR p.project_name ILIKE $${paramIndex}
                OR b.tuleap_artifact_id::text ILIKE $${paramIndex}
            )`, `%${search}%`);
        }

        if (relatedId) {
            if (relatedType === 'task') {
                addWhere(`EXISTS (SELECT 1 FROM bug_tasks bt WHERE bt.bug_id = b.id AND bt.task_id = $${paramIndex}::uuid)`, relatedId);
            } else if (relatedType === 'story' || relatedType === 'user_story') {
                addWhere(`EXISTS (
                    SELECT 1 FROM bug_tasks bt
                    JOIN tasks t ON t.id = bt.task_id
                    WHERE bt.bug_id = b.id AND t.parent_user_story_id = $${paramIndex}::uuid
                )`, relatedId);
            } else if (relatedType === 'test_case') {
                addWhere(`(
                    EXISTS (SELECT 1 FROM bug_test_cases btc WHERE btc.bug_id = b.id AND btc.test_case_id = $${paramIndex}::uuid)
                    OR EXISTS (
                        SELECT 1 FROM bug_test_executions bte
                        JOIN test_execution te ON te.id = bte.test_execution_id
                        WHERE bte.bug_id = b.id AND te.test_case_id = $${paramIndex}::uuid
                    )
                )`, relatedId);
            } else if (relatedType === 'test_execution') {
                addWhere(`EXISTS (
                    SELECT 1 FROM bug_test_executions bte
                    WHERE bte.bug_id = b.id AND bte.test_execution_id = $${paramIndex}::uuid
                )`, relatedId);
            }
        }
        ...
        const validSortFields = ['created_at', 'updated_at', 'reported_date', 'severity', 'status', 'title'];
...
router.get('/by-project/:qcProjectId', requireAuth, requirePermission('qc.bugs.view'), async (req, res) => {
    try {
        const { qcProjectId } = req.params;
        ...
        `;
        const result = await pool.query(query, [qcProjectId]);
...
router.post('/', requireAuth, requirePermission('qc.bugs.create'), async (req, res) => {
    try {
        ...
        if (linked_test_case_ids.length > 0) {
            for (const tcId of linked_test_case_ids) {
                await pool.query(
                    `INSERT INTO bug_test_cases (bug_id, test_case_id, source, relationship_type)
                     VALUES ($1, $2, 'qc', 'reveals') ON CONFLICT (bug_id, test_case_id) DO NOTHING`,
                    [bug.id, tcId]
                );
            }
        }
...
router.patch('/:id', requireAuth, requirePermission('qc.bugs.edit'), async (req, res) => {
    try {
        ...
        if (Array.isArray(req.body.linked_test_case_ids)) {
            await pool.query('DELETE FROM bug_test_cases WHERE bug_id = $1 AND source = $2', [id, 'qc']);
            for (const tcId of req.body.linked_test_case_ids) {
                await pool.query(
                    `INSERT INTO bug_test_cases (bug_id, test_case_id, source, relationship_type)
                     VALUES ($1, $2, 'qc', 'reveals') ON CONFLICT (bug_id, test_case_id) DO NOTHING`,
                    [id, tcId]
